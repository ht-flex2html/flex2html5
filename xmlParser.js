...
var XMLParser = (function () {
    function XMLParser(content) {
      ...
      this.parser.onopentag = function (tag) {
        ...
        if (HTMLElem && attributeParam && !isTagClosed) {
        ...
        if (HtmlKind.domTags.hasOwnProperty(curTagName)) {
            ...
            for(var key in tag.attributes){
              ...
              if(arrtValue){
                  ...
                  if(key == "HORIZONTALCENTER")
                      arrtValue = HTMLElem.parseHorizontal(currentDom, arrtValue);
                  if(key == "VERTICALCENTER")
                      arrtValue = HTMLElem.parseVertical(currentDom, arrtValue);
      ...
      this.parser.onclosetag = function (tagName) {
        ...
        if(HtmlKind.domTags.hasOwnProperty(tagName) && tagName == curTagName){
          isTagClosed = true;
        }
